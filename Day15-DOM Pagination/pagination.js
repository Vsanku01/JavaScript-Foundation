...
const noOfParentPages = 10;
const noOfChildPages = Math.floor(lengthOfData / noOfParentPages);
console.log(noOfChildPages);

const cssPagination = document.createElement('div');
cssPagination.classList.add('cssPagination');

document.body.append(cssPagination);

const pageTitle = document.createElement('h1')
pageTitle.classList.add('page-title')
pageTitle.innerText = 'Pagination Using DOM'

cssPagination.append(pageTitle);

const pages = document.createElement('div');
pages.classList.add('pages');

cssPagination.append(pages);
let count = 0;

for (let i = 1; i < noOfParentPages + 1; i++) {
  const page = document.createElement('div');
  page.classList.add('page');
  page.setAttribute('id', `page${i}`);
  pages.append(page);

  for (let j = noOfChildPages; j >= 1; j--) {
    count += 1;
    const title = document.createElement('div');
    title.classList.add('title');
    const anchor = document.createElement('a');
    anchor.innerText = `${data[count-1].name}`;
    // anchor.setAttribute('href', '/article7.html');
    title.append(anchor);
    page.append(title);
  }
}
...
cssPagination.append(pageNav);

for (let i = 1; i <= noOfParentPages; i++) {
  const a = document.createElement('a');
  a.classList.add('pageNumber');
  a.setAttribute('href', `#page${i}`);
  a.setAttribute('id', `pg-${i}`);
  a.innerText = i;
  pageNav.append(a);
  a.setAttribute('onclick', `updateColor(${i})`);
}

let flag = false;
function updateColor(i) {
  const ref = document.getElementById(`pg-${i}`);
  console.log(flag)
  if (!ref.classList.contains('active') && flag === false) {
    ref.classList.add('active');
    flag = true;
  } 
  else if(!ref.classList.contains('active') && flag === true){
    for(let j =1 ; j<= noOfParentPages; j++){
      const tempRef = document.getElementById(`pg-${j}`);
      if(j !== i && (tempRef.classList.contains('active'))){
        tempRef.classList.remove('active');
        ref.classList.add('active');
      }
    }
  }
